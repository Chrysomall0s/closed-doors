...
func _ready() -> void:
	...
	if player_camera == null:
		...
		assert(players != null, "Player not found!")
		for p in players:
			if p is CharacterBody3D:
				var cam = p.find_child("Camera3D", true, false)
				if cam:
					player_camera = cam
					break
	
		assert(player_camera != null, "Player camera not found!")
...
func _setup_cameras() -> void:
	...
	if exit_portal != null:
		...
		var source_env: Environment = player_camera.environment

		if source_env == null:
			source_env = player_camera.get_world_3d().environment

		if source_env == null:
			push_warning("No environment found, creating default one")
			source_env = Environment.new()

		var adjusted_env: Environment = source_env.duplicate()
